...
function calcAge(date) {
    ...
    const now = new Date()
    const age = Math.floor(now.getTime() / round) - Math.floor(date / round)
...
class Menu {
    ...
    /**
     * Creates context menu corresponding to handle list.
     * @param {FbMetadbHandleList} metadb_handles
     */
    initContextMenu(metadb_handles) {
        if (metadb_handles) {
            this.contextMenu = true
            this.cMenuManager = fb.CreateContextMenuManager()
            this.cMenuManager.InitContext(metadb_handles)
            this.cMenuManager.BuildMenu(this.menu, 2000)
        }
    };
     ...
     * @param {?boolean=} [disabled=false]
     */
    addToggleItem(label, propertiesObj, propertyName, callback = () => {
    }, disabled = false) {
    ...
    doCallback(idx) {
        if (idx > menuStartIndex && _MenuCallbacks[idx]) {
            ...
            this.menuManager = null
        } else if (this.contextMenu && idx) {
            this.cMenuManager.ExecuteByID(idx - 2000)
            this.cMenuManager = null
        }
        this.menu = null
