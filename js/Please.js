...
function _Please() {
	...
	this.PHI = 0.618033988749895;

	this.make_color_default = {
		hue: null,
		saturation: null,
		value: null,
		base_color: "",
	...
	this.make_scheme_default = {
		scheme_type: "analogous",
		format: "hex",
	};

	this.make_contrast_default = {
		golden: false,
		format: "hex",
	};

	this.random_int = (min, max, randomiser) => {
		var random = Math.random;
		if (randomiser instanceof this.RC4Random) {
			random = randomiser.random;
		}
		return Math.floor(random() * (max - min + 1)) + min;
	};

	this.random_float = (min, max, randomiser) => {
		var random = Math.random;
		if (randomiser instanceof this.RC4Random) {
			random = randomiser.random;
		}
	...
	this.convert_to_format = (format_string, array) => {
		var i;
		switch (format_string) {
			case "hex":
				for (i = 0; i < array.length; i++) {
					array[i] = this.HSV_to_HEX(array[i]);
				}
				break;
			case "rgb":
				for (i = 0; i < array.length; i++) {
					array[i] = this.HSV_to_RGB(array[i]);
				}
				break;
			case "rgb-string":
				for (i = 0; i < array.length; i++) {
					var raw_rgb = this.HSV_to_RGB(array[i]);
	...
	this.contrast_quotient = (HSV) => {
		var RGB = this.HSV_to_RGB(HSV);
		var YIQ = (RGB.r * 299 + RGB.g * 587 + RGB.b * 114) / 1000;
		return YIQ >= 128 ? "dark" : "light";
	};

	this.copy_object = (object) => {
		var copy = {};
	...
	this.NAME_to_HEX = function (name) {
		name = name.toLowerCase();
		if (name in this.color_data) {
			return this.color_data[name];
		} else {
			console.error("Color name not recognized.");
		}
	};

	this.NAME_to_RGB = function (name) {
		return this.HEX_to_RGB(this.NAME_to_HEX(name));
	};

	this.NAME_to_HSV = function (name) {
		return this.HEX_to_HSV(this.NAME_to_HEX(name));
	};
	...
	this.make_scheme = function (HSV, options) {
		//this.clone base please options
		var scheme_options = this.copy_object(this.make_scheme_default),
			adjusted,
			secondary,
			adjusted_h,
			adjusted_s,
		...
		switch (scheme_options.scheme_type.toLowerCase()) {
			...
			case "mono":
				for (i = 1; i <= 2; i++) {
					adjusted = this.clone(HSV);

					adjusted_s = adjusted.s + 0.1 * i;
					adjusted_s = this.clamp(adjusted_s, 0, 1);

					adjusted_v = adjusted.v + 0.1 * i;
					adjusted_v = this.clamp(adjusted_v, 0, 1);
		...
		this.convert_to_format(scheme_options.format.toLowerCase(), scheme);
		return scheme;
	};

	//accepts options object returns list or single color
	this.make_color = function (options) {
		var color = [],
			//this.clone base please options
			color_options = this.copy_object(this.make_color_default),
			base_color = null;
		...
		if (typeof color_options.seed === "string") {
			randomiser = new this.RC4Random(color_options.seed);
		}
		...
		if (color_options.base_color.length > 0) {
			...
			if (color_options.base_color.match(/^#?([0-9a-f]{3})([0-9a-f]{3})?$/i)) {
				base_color = this.HEX_to_HSV(color_options.base_color);
			} else {
				base_color = this.NAME_to_HSV(color_options.base_color);
			}
		}
		for (var i = 0; i < color_options.colors_returned; i++) {
			var random_hue = this.random_int(0, 360, randomiser),
				hue,
				saturation,
				value;
			if (base_color !== null) {
				hue = this.clamp(this.random_int(base_color.h - 5, base_color.h + 5, randomiser), 0, 360);
				//fix for black/gray as a base color returning reds
				if (base_color.s === 0) {
					saturation = 0;
				} else {
					saturation = this.random_float(0.4, 0.85, randomiser);
				}
				value = this.random_float(0.4, 0.85, randomiser);
				color.push({
					h: hue,
					s: saturation,
					v: value,
				...
				//make hue goldennnnnnnn
					...
					hue = (random_hue + random_hue / this.PHI) % 360;
				} else if (color_options.hue === null || color_options.full_random === true) {
					hue = random_hue;
				} else {
					hue = this.clamp(color_options.hue, 0, 360);
				}
				//set saturation
				if (color_options.greyscale === true || color_options.grayscale === true) {
					saturation = 0; //if they want greyscale no saturation allowed
				} else if (color_options.full_random === true) {
					saturation = this.random_float(0, 1, randomiser);
				} else if (color_options.saturation === null) {
					saturation = 0.4;
				} else {
					saturation = this.clamp(color_options.saturation, 0, 1);
				}
				//set value
				if (color_options.full_random === true) {
					value = this.random_float(0, 1, randomiser);
				} else if (color_options.greyscale === true || color_options.grayscale === true) {
					value = this.random_float(0.15, 0.75, randomiser);
				} else if (color_options.value === null) {
					value = 0.75;
				} else {
					value = this.clamp(color_options.value, 0, 1);
				}
				color.push({ h: hue, s: saturation, v: value });
			}
		}
		//output options based on format
		this.convert_to_format(color_options.format.toLowerCase(), color);

		return color;
	};
	//accepts HSV object returns contrasting color
	this.make_contrast = function (HSV, options) {
		//this.clone base please options
		var contrast_options = this.copy_object(this.make_contrast_default);
		...
		var contrast,
			value_range = this.contrast_quotient(HSV),
			adjusted_hue;

		if (contrast_options.golden === true) {
			adjusted_hue = (HSV.h * (1 + this.PHI)) % 360;
		} else {
			var contrast_base = this.make_scheme(HSV, {
				scheme_type: "complementary",
				format: "hsv",
			})[1];
			adjusted_hue = this.clamp(contrast_base.h - 30, 0, 360);
		}
		var adjusted_value;
		if (value_range === "dark") {
			adjusted_value = this.clamp(HSV.v - 0.25, 0, 1);
		...
		this.convert_to_format(contrast_options.format.toLowerCase(), contrast);
		return contrast[0];
	};
}